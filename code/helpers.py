...
def angle_between_vectors(v0, v1, deg):
    ...
    divisor = np.sqrt(np.square(v0.x) + np.square(v0.y) + np.square(v0.z)) * np.sqrt(np.square(v1.x) + np.square(v1.y) + np.square(v1.z))
    factor = 180 / np.pi if deg else 1
    return np.acos(dividend / divisor) * factor
